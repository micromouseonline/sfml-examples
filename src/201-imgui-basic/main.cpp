...
int main() {
    ...
    for (int i = 0; i < mazeCount; i++) {
        items.push_back(mazeList[i].title);
    }
    ...
    while (window.isOpen()) {
        ...
        bool maze_changed = ImGui::Combo("Maze", &maze_index, items.data(), (int) items.size());
        ImGui::End();
        if (maze_changed) {
            std::cout << mazeList[maze_index].title << "   " << mazeList[maze_index].size << std::endl;
        }
